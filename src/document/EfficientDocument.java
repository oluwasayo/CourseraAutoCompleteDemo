...
public class EfficientDocument extends Document {
    ...
    private void processText() {
      ...
      //  System.out.println("List of tokens " + tokens);
        ...
        if(size>0){
          //  System.out.println("last element "+tokens.get(size-1));
        if(!(tokens.get(size-1).endsWith("?"))&&!(tokens.get(size-1).endsWith("."))&&!(tokens.get(size-1).endsWith("!"))){
            numSentences++;
        }
        }
    }
...
testCase(new EfficientDocument(", 3 out of 4 times. This is line 3? Yes.!"), 6, 3, 2);
    }
